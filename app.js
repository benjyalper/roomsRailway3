        ...
        try {
            ...
            if (recurringEvent) {
                ...
                for (let i = 0; i < `${recurringNum}`; i++) {
                    ...
                    await connection.execute('INSERT INTO selected_dates_3 (selected_date, names, color, startTime, endTime, roomNumber, recurringEvent, recurringNum) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [nextDate, names, selectedColor, startTime, endTime, roomNumber, recurringEvent, recurringNum]);
                }
            } else {
...
app.post('/checkRecurringEvent', async (req, res) => {
    try {
        ...
        const recurringQuery = 'SELECT * FROM selected_dates_3 WHERE selected_date = ? AND roomNumber = ? AND startTime = ? AND recurringEvent = true';
        ...
        const nonRecurringQuery = 'SELECT * FROM selected_dates_3 WHERE selected_date = ? AND roomNumber = ? AND startTime = ? AND recurringEvent = false';
...
app.get('/fetchDataByDate', async (req, res) => {
    try {
        ...
        const connection = await pool.getConnection();
        const [rows] = await connection.execute('SELECT selected_date, names, color, startTime, endTime, roomNumber FROM selected_dates_3 WHERE selected_date = ?', [lookupDate]);
        connection.release();
